...
if(quizuser.quiz){
  ...
  body.innerHTML=`test already completed to see result <a href="./result.html">click</a>`;
...
let storage = [
  ...
  function legends(){
   let legendCount=document.querySelector("*#legends");
   let answer=legendCount.querySelectorAll("span")[0];
   let NotAnswer=legendCount.querySelectorAll("span")[1];
   let marked=legendCount.querySelectorAll("span")[2];
   let notVisited=legendCount.querySelectorAll("span")[3];
   let answerCount=0;
  ...
  submitBtn.addEventListener("click",()=>{
    let conf =confirm("Are you sure wait to submit ?");
    if(conf){
      quizuser.quiz=storage;
      localStorage.setItem("quizuser",JSON.stringify(quizuser));
      details=details.filter((e)=>{
